...
class Receipt:
    ...
    def __init__(self, uid, pid):
        ...
        self.p.set("left","b")
        self.p.text("\nBegin results for participant number " + str(self.pid) + ":\n\n")
    ...
    def finalize(self):
        ...
        qlines = textwrap.wrap(text, 40)
        for line in qlines:
            self.p.text(line + "\n")

        self.p.text("\nSign Below\n\n")

        self.p.set("left","b",'NORMAL',2,2)
        self.p.text("_____________________\n")


    def addQuestionAnswer(self, q, a):
        self.qanswered += 1
        self.p.set("left","b", "NORMAL",1,1, 0, True)

        qlines = textwrap.wrap(q, 40)
        for line in qlines:
            self.p.text(line + "\n")
